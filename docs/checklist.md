...
| `scan_film_runaway.py` | Does the model blow up 10s-100s of times normal peak at a narrow (knob-corner × transient) combination the training excitation under-covered? | Runs automatically as a post-training WARN step in `run_pipeline.py` **if `--film-reference` is set** (a clip distinct from the training excitation — a real recording or a synthesized capture sweep both work — not derivable from the training excitation itself, which is exactly the gap that let two published models ship with this bug unnoticed). Skipped with a note if unconfigured. Skip flag: `--skip-film-runaway-check`. |
