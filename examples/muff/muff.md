# Electro-Harmonix Big Muff Pi — V1 ("66#5")

Our own transcription. **Topology verified node-for-node against the ElectroSmash
Big Muff Pi analysis** (authoritative); component **values are the V1 66#5**
(BigMuffPage / Kit Rae trace). The early hand-wired "triangle" Big Muff — a
...
## Topology (ElectroSmash-verified)
...
cap** — a flat ~13 dB gain stage that recovers the passive-tone loss (per
ElectroSmash).
...
- **Clipping (Q3, Q2):** `collector → 1 µF (C6/C7) → anti-parallel 1N914 pair →
  ...
  (blocking the DC bias) — this is the actual Big Muff clipping mechanism, and it
...
## Lessons learned (5 rounds of comparison vs ElectroSmash)
Built iteratively, re-comparing against the **authoritative ElectroSmash analysis
PDF node-by-node each round** (not from memory). Four rounds found real bugs; the
fifth confirmed convergence. The traps, in order found:

1. **Clipping is AC-coupled, not diodes-across-C-B.** The Big Muff clips via
...
3. **The output recovery Q1 does NOT follow the other stages' pattern.** Q4/Q3/Q2
   ...
   exactly what ES describes.
4. **All three pots are linear taper** ("100K Lin" / 66#5 "Linear taper
   potentiometers"). Don't default Volume to logarithmic.

**Verification methodology that worked** (reusable for any pedal/amp build):
- Fetch the authoritative reference (ES analysis, schematic PDF) and diff
...
## 66#5 vs the ElectroSmash V3 study (same topology, different values)
- R13/R11 = **39 k** (V3: 10 k) · emitters R21/R10/R22 = **120 Ω** (V3: 150/100 Ω)
  · Miller caps C10/C11/C12 = **500 pF** (V3: 470 pF) · R14 = **100 k** (V3: 47 k)
  · several couplings **.68 µF** (V3: some 1 µF / 100 nF) · R5/R8 = **27 k** (V3:
  22 k / 39 k). These are the documented V1 66#5 values.

## Model params
2N5133 → silicon NPN (IS = 1 pA, BF = 300 — high hFE for sustain); 1N914 →
IS = 2.52 nA / n = 1.752 (≈ 1N4148). Reuses the DS-1 BJT (`Q`) + Diode (`D`)
support. `Rout` (1 MΩ) is an output-reference resistor, not a schematic part.
