...
class TestGainVolumeKnobGrid:
    ...
    def test_default_matches_documented_seven_point_anchor_set(self):
...
# ------------------------------------------- LOUD backend failure, measured rather than asserted
#
# The scaffold already renders the circuit (measure_truncation at 2/4/8 against a 32 reference),
# so it is better placed than any other tool to DISCOVER that a backend cannot hold it. It used
# to catch every exception, blame it on a possibly-unbuilt oracle, and downgrade to a placeholder
# oversample -- so it could watch the MT-2 diverge and still write backend = "livespice" without
# comment. A divergence is an observation, not an opinion, so the sidecar it justifies is
# generated, not hand-written.

import tomllib
from pathlib import Path

import scaffold_config as sc
...
def test_a_divergence_is_distinguished_from_a_setup_failure(err):
    assert sc._is_convergence_failure(err)


@pytest.mark.parametrize("err", [
    "no such file or directory: livespice_cli",
    "permission denied",
])
def test_a_missing_oracle_is_not_read_as_a_backend_verdict(err):
    # Blaming a setup problem on the circuit would be worse than the bug this replaced.
    assert not sc._is_convergence_failure(err)


def test_generated_sidecar_is_valid_toml_declaring_the_backend_invalid(tmp_path):
    schx = tmp_path / "Stiff Pedal.schx"
    stub = sc._sidecar_stub(schx, "livespice", "Circuit.SimulationDiverged near t=2.65s (exit 134)")
    d = tomllib.loads(stub)
    assert d["livespice"]["valid"] is False
    assert "SimulationDiverged" in d["livespice"]["reason"]


def test_generated_sidecar_records_that_it_was_measured_and_asks_for_review(tmp_path):
    # It must not read as a settled verdict: a divergence can be a probe artefact, and the
    # cheapest thing to rule out first is too-few Newton iterations reading as stiffness.
    stub = sc._sidecar_stub(tmp_path / "P.schx", "livespice", "diverged")
    assert "REVIEW BEFORE COMMITTING" in stub
    assert "iterations" in stub
    assert "Generated by scaffold_config.py" in stub


def test_generated_sidecar_is_accepted_by_the_gate_that_will_read_it(tmp_path):
    # The two halves must agree: a file the scaffold writes has to be one check_backend
    # enforces, and one the sidecar lint passes without warnings.
    import io
    import contextlib
    ...
    schx.with_suffix(".backends.toml").write_text(
        sc._sidecar_stub(schx, "livespice", "Circuit.SimulationDiverged near t=2.65s"))

    class _Ap:
        def __init__(self): self.errors = []
        def error(self, m): self.errors.append(m); raise SystemExit(2)

    ap, err = _Ap(), io.StringIO()
    with pytest.raises(SystemExit):
        with contextlib.redirect_stderr(err):
            g.check_backend(schx, "livespice", ap)
    assert "CANNOT be rendered faithfully" in ap.errors[0]
    assert "WARNING" not in err.getvalue(), "the generated file must lint clean"
